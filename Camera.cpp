...
void CCamera::DSCreateShaderVariables(ID3D11Device *pd3dDevice)
{
	...
	ZeroMemory(&bd, sizeof(bd));
	bd.Usage = D3D11_USAGE_DYNAMIC;
	bd.ByteWidth = sizeof(DS_CB_CAMERA);
	bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	pd3dDevice->CreateBuffer(&bd, NULL, &m_pd3dcbCameraInDS);
}

void CCamera::DSUpdateShaderVariables(ID3D11DeviceContext *pd3dDeviceContext)
{
	D3D11_MAPPED_SUBRESOURCE d3dMappedResource;
	/*상수 버퍼의 메모리 주소를 가져와서 카메라 변환 행렬과 투영 변환 행렬을 복사한다. 쉐이더에서 행렬의 행과 열이 바뀌는 것에 주의하라.*/
	pd3dDeviceContext->Map(m_pd3dcbCameraInDS, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedResource);
	DS_CB_CAMERA *pcbViewProjection = (DS_CB_CAMERA *)d3dMappedResource.pData;
	D3DXMatrixTranspose(&pcbViewProjection->m_d3dxmtxView, &m_d3dxmtxView);
	D3DXMatrixTranspose(&pcbViewProjection->m_d3dxmtxProjection, &m_d3dxmtxProjection);
	pd3dDeviceContext->Unmap(m_pd3dcbCameraInDS, 0);

	//상수 버퍼를 슬롯(VS_SLOT_CAMERA)에 설정한다.
	pd3dDeviceContext->DSSetConstantBuffers(VS_SLOT_CAMERA, 1, &m_pd3dcbCameraInDS);
}


void CCamera::DSClearShaderVariables(ID3D11DeviceContext *pd3dDeviceContext)
{
	pd3dDeviceContext->DSSetConstantBuffers(NULL, NULL, NULL);
}
...
void CFirstPersonCamera::Rotate(float x, float y, float z)
{
	...
	if (m_pPlayer && (z != 0.0f))
	{
		...
		cout << "카메라 위치" << m_d3dxvPosition.x << "  " << m_d3dxvPosition.y << "  " << m_d3dxvPosition.z << endl;
